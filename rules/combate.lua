local combate = [[
- Combate -
0. Testes de Oposição:
Para manter a agilidade e o foco na ação, o sistema utiliza uma regra universal para todos os testes realizados contra um oponente, seja para atacar, usar uma habilidade ou impor uma condição.
● Rolagem Padrão: Todos os testes são feitos rolando 1d20. Não há perícias envolvidas. Bônus específicos podem ser concedidos por habilidades ou equipamentos.
● Cálculo da Dificuldade: A dificuldade para qualquer teste contra um alvo é sempre igual ao Modificador de Energia desse alvo.
1. Iniciativa:
No início de um combate, todos os participantes rolam para determinar a ordem de ação. A Iniciativa é uma medida da velocidade de processamento e dos reflexos otimizados da sua classe.
...
● Surto de Ação (Action Surge): Você pode abdicar da sua Ação de Movimento no seu turno para preparar sua Ação Principal. Ao fazer isso, você pode executar essa Ação Principal preparada a qualquer momento antes do início do seu próximo turno, como uma reação a um evento ou simplesmente para agir no turno de outro personagem.
3. Ataque:
